...
async def cmd_start_panel(message: Message, state: FSMContext, api_client: MarzneshinAPI):
    fsm_data = await state.get_data() or {}
    old_panel_id = fsm_data.get("main_panel_id")

    if old_panel_id:
        try:
            await message.bot.delete_message(chat_id=message.chat.id, message_id=old_panel_id)
        except Exception:
            pass
    ...
    panel_message = await message.answer("⏳ Fetching latest panel stats...")

    text, keyboard = await _get_dashboard_content(api_client)
    await panel_message.edit_text(text, reply_markup=keyboard, parse_mode="Markdown")

    await state.update_data(main_panel_id=panel_message.message_id)
...
async def cb_back_to_main_menu(callback: CallbackQuery, state: FSMContext, api_client: MarzneshinAPI):
    ...
    if old_panel_id and old_panel_id != callback.message.message_id:
        try:
            await callback.bot.delete_message(chat_id=callback.message.chat.id, message_id=old_panel_id)
        except Exception:
            pass
    ...
    try:
        await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="Markdown")
        await state.update_data(main_panel_id=callback.message.message_id)
